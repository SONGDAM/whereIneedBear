//items.look[index]의 값을 다양하게 설정해야함.
...
let filtered = items.filter((i) => i.temperature == nowTemp);
...
document.getElementById('tempSelect').addEventListener('click', function () {
  if (tempSelect.children.value === 'nowTemp') {
...
//버튼별(룩)별 이미지 띄우기
function btnLook(s) {
  const filter_look = items.filter((el, i) => items[i].look == s.value);
  document.getElementById('imgItem').innerHTML = `
